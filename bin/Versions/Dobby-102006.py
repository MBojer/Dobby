    ...
    def On_Message(cls, id, Payload):
        ...
        if Action == 0:
            ...
            if Action == Alert_State:
                ...
                # Send Email
                Send_Mail(Alert_Target_Email, Mail_Trigger.Build_Trigger_Message(0, Trigger_Info, Payload))
        ...
        # Trigger
            ...
            if Action == Alert_State:
                ...
                # Send Email
                Send_Mail(Alert_Target_Email, Mail_Trigger.Build_Trigger_Message(1, Trigger_Info, Payload))
...
# ---------------------------------------- Timeout Trigger ----------------------------------------
def Send_Alert(Subject, Body, Value=None, Mail=None, MQTT=None, Push=None):
    if Mail is None and MQTT is None and Push is None:
        return

    # Add value to body if value is set
    ...
    if Mail is not None:
        Trigger_Message = MIMEMultipart()
        Trigger_Message['From'] = Dobby_Config['Mail_Trigger_SMTP_Sender']
        Trigger_Message['To'] = Mail
        Trigger_Message['Subject'] = Subject
    ...
    if MQTT is not None:
        ...
        for Target_Topic in Target_List:
            # Publish message
            MQTT_Client.publish(Target_Topic, payload=Body, qos=0, retain=False)
            # Log event
            Log("Debug", "System", "MQTT", 'Publish - Topic: ' + str(Target_Topic) + " Payload: " + str(Body))
            

    if Push is not None:
        Send_Push(Push, Subject, Body)
...
def Send_Mail(To, Message):
    ...
    for To_Email in Target_List:
        Mail_Server.sendmail(Dobby_Config['Mail_Trigger_SMTP_Sender'], To_Email, str(Message))
    ...
    def On_Message(cls, id, Payload):
        ...
        if Action == 0:
            ...
            if Action == Alert_State:
                ...
                # Send Push Notification
                Send_Push(Alert_Target, Alert_Subject, Alert_Payload_Clear + " - Value: " + str(Payload))
        ...
        # Trigger
            ...
            if Action == Alert_State:
                ...
                # Send Push Notification 
                Send_Push(Alert_Target, Alert_Subject, Alert_Payload_Trigger + " - Value: " + str(Payload))

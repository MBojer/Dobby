...
class Action_Trigger():
    ...
    def On_Message(cls, id, Payload):
        ...
        db_Curser.execute("SELECT Name, `Alert State`, `MQTT Payload Clear`, `MQTT Payload Trigger`, `Alert Target`, `Alert Payload Clear`, `Alert Payload Trigger` FROM Dobby.Action_Trigger WHERE id=" + str(id) + ";")
        Trigger_Info = db_Curser.fetchone()
...
def gBridge_Trigger_Local(id, Topic, Payload):
    ...
    MQTT_Client_gBridge.Publish_Reply(id, Payload)
...
class gBridge_Trigger():
    ...
    def Publish_Reply(self, id, Payload):
    ...
    def Start_Trigger(self, id):
        ...
        # Add topic to topic tict
        MQTT_Add_Sub_Topic(str(self.Active_Triggers[id]["MQTT Target"]) + "/State", 'gBridge Trigger', id)
        # Subscribe
        MQTT_Client.subscribe(str(self.Active_Triggers[id]["MQTT Target"]) + "/State")
        # Register callbacks
        MQTT_Client.message_callback_add(str(self.Active_Triggers[id]["MQTT Target"] + "/State"), MQTT_On_Message_Callback)
        ...
        if Do_Something == True:
            ...
            for Function in Target_Function:
                ...
                if Retained is 0:
                    ...
                    elif Function[0] == "Push Trigger":
                        Push_Trigger.On_Message(Function[1], Payload)
...
class Push_Trigger():
    ...
    def __init__(self):
        # Log event
        Log("Info", "Push Trigger", "Checker", "Initializing")
    ...
    def Checker(self):
        ...
        while True:
            ...
            # Get id and Last Modified to check if Push Triggers needs to be started
            db_Curser.execute("SELECT id, Last_Modified FROM Dobby.`Push_Trigger` WHERE Enabled=1;")
            Push_Info = db_Curser.fetchall()

            for i in range(len(Push_Info)):
                # Push_Info[i][0] - id
                # Push_Info[i][1] - Last Modified

                # Check if the trigger is in the Active_Triggers dict
                if Push_Info[i][0] in self.Active_Triggers:
                    
                    # Check if last modified changed
                    if self.Active_Triggers[Push_Info[i][0]] != Push_Info[i][1]:
                        self.Active_Triggers[Push_Info[i][0]] = Push_Info[i][1]
                        self.Restart_Trigger(Push_Info[i][0])
                        
                # If not add then to the list and start the trigger
                else:
                    self.Active_Triggers[Push_Info[i][0]] = Push_Info[i][1]
                    # Start the trigger
                    self.Start_Trigger(Push_Info[i][0])
    ...
    def Start_Trigger(self, id):
        ...
        db_Curser.execute("SELECT Name, `MQTT Target` FROM Dobby.`Push_Trigger` WHERE id="+ str(id) + ";")
        Trigger_Info = db_Curser.fetchone()
        # Trigger_Info[0] - Name
        # Trigger_Info[1] - Target
        ...
        # Log Event
        Log("Debug", "Push Trigger", str(Trigger_Info[0]), "Starting")
        Log("Debug", "Push Trigger", str(Trigger_Info[0]), "Subscribing to: '" + str(Trigger_Info[1]) + "'")
        # Add topic to topic tict
        MQTT_Add_Sub_Topic(str(Trigger_Info[1]), 'Push Trigger', id)
        # Subscribe
        MQTT_Client.subscribe(str(Trigger_Info[1]))
        # Register callbacks
        MQTT_Client.message_callback_add(str(Trigger_Info[1]), MQTT_On_Message_Callback)
    ...
    def Stop_Trigger(self, id):
        ...
        Trigger_Info = self.Get_Trigger_Info(id)
         ...
         # Log Event
        Log("Debug", "Push Trigger", str(Trigger_Info[0]), "Stopping")
        Log("Debug", "Push Trigger", str(Trigger_Info[0]), "Unsubscribing from: '" + str(Trigger_Info[1]) + "'")

        MQTT_Del_Sub_Topic(Trigger_Info[1], 'Push Trigger', id)
    ...
    @classmethod
    def Send_Push(cls, Target, Subject, Message, Name):
        # Remove any spaces
        ...
        for Target_Push in Target_List:
            ...
            Log("Debug", "Push Trigger", str(Name), 'Status: ' + str(r.status_code))
    ...
    def On_Message(cls, id, Payload):
        ...
        db_Curser.execute("SELECT Name, `Alert State`, `MQTT Payload Clear`, `MQTT Payload Trigger`, `Alert Target`, `Alert Subject`, `Alert Payload Clear`, `Alert Payload Trigger` FROM Dobby.Push_Trigger WHERE id=" + str(id) + ";")
        Trigger_Info = db_Curser.fetchone()
        ...
        if float(MQTT_Payload_Clear) == float(MQTT_Payload_Trigger):
            Log("Error", "Push Trigger", str(Name), 'Clear and Trigger payload is the same value')
            Close_db(db_Connection, db_Curser)
            return
        ...
        if Action == 0:
            # Check agains current alert state
            if Action == Alert_State:
                Log("Debug", "Push Trigger", str(Name), 'Already cleared ignoring new clear value: ' + str(Payload))
            else:
                Trigger_Change = True

                # Send Push Notification
                Push_Trigger.Send_Push(Alert_Target, Alert_Subject, Alert_Payload_Clear + " - Value: " + str(Payload), Name)
                Log("Info", "Push Trigger", str(Name), 'Cleared at: ' + str(Payload) + " Target: " + str(MQTT_Payload_Clear))

        # Trigger
        elif Action == 1:
            # Check agains current alert state
            if Action == Alert_State:
                Log("Debug", "Push Trigger", str(Name), 'Already triggered ignoring new trigger value: ' + str(Payload))
            else:
                Trigger_Change = True

                # Send Push Notification 
                Push_Trigger.Send_Push(Alert_Target, Alert_Subject, Alert_Payload_Trigger + " - Value: " + str(Payload), Name)
                Log("Info", "Push Trigger", str(Name), 'Triggered at: ' + str(Payload) + " Target: " + str(MQTT_Payload_Trigger))

        # In-between value
        elif Action == 2:
            Log("Debug", "Push Trigger", str(Name), 'In-between value received: ' + str(Payload))

        if Trigger_Change is True:
            # Change Alert_State
            db_Curser.execute("UPDATE `Dobby`.`Push_Trigger` SET `Alert State`='" + str(Action) + "' WHERE `id`='" + str(id) + "';")
            # Update Triggered_DateTime
            db_Curser.execute("UPDATE `Dobby`.`Push_Trigger` SET `Triggered DateTime`='" + str(datetime.datetime.now()) + "' WHERE `id`='" + str(id) + "';")

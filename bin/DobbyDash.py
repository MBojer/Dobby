...
def Generate_Config_List_System(Config_Dropdown, Config_Dropdown_Line, db_Curser=None, Sort_By=[]):
    ...
    # Convert Dash's sort_by to a sort by SQL string
    if Sort_By != []:
        Sort_By = " ORDER BY `" + Sort_By[0]['column_id'] + "` " + Sort_By[0]['direction'] 
    # Set Sort_By to '' or else the select below will fail 
    else:
        Sort_By = ''
    ...
    try:
        ...
        if Config_Dropdown_Line != "-- New Entry --":
            db_Curser.execute("SELECT * FROM Dobby." + Config_Dropdown.replace(" ", "_") + " WHERE `Target`='" + Config_Dropdown_Line + "'" + Sort_By + ";")
...
def Generate_Config_List_Functions(Config_Dropdown, Config_Dropdown_Line, db_Curser=None, Sort_By=[]):
    ...
    try:
        ...
        if Config_Dropdown_Line != "-- New Entry --":
            db_Curser.execute("SELECT * FROM Dobby." + Config_Dropdown.replace(" ", "_") + " WHERE `Function`='" + Config_Dropdown_Line + "'" + Sort_By + ";")
            Values = db_Curser.fetchall()
        # New Entry
        else:
            db_Curser.execute("SELECT Column_Default, IS_NULLABLE FROM Information_Schema.Columns WHERE table_schema='Dobby' AND table_name='" + Config_Dropdown.replace(" ", "_") + "';")
...
def Generate_Config_List(Config_Dropdown, Config_Dropdown_Line, db_Curser=None, Sort_By=[]):
    ...
    # Convert Dash's sort_by to a sort by SQL string
    if Sort_By != []:
        # use settings value name in stea of cloums id
        Sort_By = " ORDER BY `" + Sort_By[0]['column_id'] + "` " + Sort_By[0]['direction'] 
    # Set Sort_By to '' or else the select below will fail 
    else:
        Sort_By = ''
    ...
    try:
        ...
        if Config_Dropdown_Line != "-- New Entry --":
            db_Curser.execute("SELECT * FROM Dobby." + Config_Dropdown.replace(" ", "_") + " WHERE `" + Name_Field_String + "`='" + Config_Dropdown_Line + "'" + Sort_By + ";")
...
def Config_Tab_Dropdown_List():
    ...
    return ['Alert Targets', 'Alert Trigger', 'Action Trigger', 'APC Monitor', 'Counters', 'DashButtons', 'DeviceConfig', 'Dobby Assistant', 'gBridge Trigger', 'Log Trigger', 'MQTT Functions', 'Push Trigger', 'Spammer', 'SystemConfig', 'Timeouts', 'Users']
...
#     static_folder = os.path.join(os.getcwd(), '../DashStatic')
#     print static_folder
...
def Config_Tab_Variables(Config_Dropdown, Config_Dropdown_Line, Config_Read, Config_Delete_Row, Config_Save, Config_Push, Config_Table, Config_Tab_Variables):
    ...
    if int(Config_Tab_Variables.get('Config_Push', 0)) != int(Config_Push):
        if Config_Dropdown == 'DeviceConfig' and Config_Dropdown_Line != None:
            # Get the device to request a new config
            MQTT_Publish(System_Header + "/Commands/" + str(Config_Dropdown_Line) + "/FSConfig", 'Request')
    ...
    # Save
        ...
        if Config_Tab_Variables['Config_Dropdown_Line'] != "None":
            ...
            for Current_Config_Row in Current_Config:
                ...
                if Config_Table[i]['Value'] == '' or Config_Table[i]['Value'] == None:
                    # FIX - Find a way to set db cell to null
...
@app.callback(
    ...
    [
        ...
        Input('Config_Read', 'n_clicks'),
        Input('Config_Delete_Row', 'n_clicks'),
        Input('Config_Table', 'sort_by'),
        ],
    [
        State('Config_Tab_Variables', 'children'),
    ]
    )
def Config_Tab_Table(Config_Dropdown, Config_Dropdown_Line, Config_Read, Config_Delete_Row, Config_Sort_By, Config_Tab_Variables):
    ...
    try:
        ...
        if Config_Dropdown == "MQTT Functions":
            return Generate_Config_List_Functions(Config_Dropdown, Config_Dropdown_Line, None, Config_Sort_By)
        elif Config_Dropdown == "SystemConfig":
            return Generate_Config_List_System(Config_Dropdown, Config_Dropdown_Line, None, Config_Sort_By)
        else:
            return Generate_Config_List(Config_Dropdown, Config_Dropdown_Line, None, Config_Sort_By)
...
def Device_Config_Tab_Config_Show(Devices_Tab_Variables, Device_Config_Save, Device_Config_Table):
    ...
    # ======================================== Send Config ========================================
        ...
        Devices_Tab_Variables['Last_Click'] = None
        MQTT.single(System_Header + "/Commands/Dobby/Config", Devices_Tab_Variables['Device_Config_Dropdown'] + ",0,MQTT", hostname=MQTT_Broker, port=MQTT_Port, auth={'username': MQTT_Username, 'password': MQTT_Password})
...
if __name__ == '__main__':
    app.run_server(debug=False, host='0.0.0.0', ssl_context=('/etc/Dobby/Cert/cert.pem', '/etc/Dobby/Cert/key.pem'))

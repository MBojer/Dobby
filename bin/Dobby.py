...
class APC_Monitor:
    ...
    class Agent:
        def __init__(self, id):
            ...
            db_Curser.execute("SELECT Name, Enabled, Refresh_Rate, IP, Username, Password, Next_Refresh, Last_Refresh, Last_Modified FROM Dobby.APC_Monitor WHERE id='" + str(self.id) + "'")
            ...
            self.Refresh_Rate = float(APC_Monitor_Info[2] * 60)
            self.IP = APC_Monitor_Info[3]
            self.Username = APC_Monitor_Info[4]
            self.Password = APC_Monitor_Info[5]
            self.Next_Refresh = APC_Monitor_Info[6]
            self.Last_Refresh = APC_Monitor_Info[7]
            self.Last_Modified = APC_Monitor_Info[8]
        ...
        def Run(self):
            ...
            while True:
                ...
                if self.Next_Refresh < datetime.datetime.now():
                    Log("Debug", "APC Monitor", self.Name, "Refresh Started")

                    self.Next_Refresh = datetime.datetime.now() + timedelta(seconds=self.Refresh_Rate)
                    ...
                    db_Curser.execute("UPDATE `Dobby`.`APC_Monitor` SET `Next_Refresh`='" + str(self.Next_Refresh) + "', `Last_Refresh`='" + str(datetime.datetime.now()) + "' WHERE id = '" + str(self.id) + "';")
                ...
                while self.Next_Refresh > datetime.datetime.now():
                    time.sleep(APC_Monitor.Loop_Delay)

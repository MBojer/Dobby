    ...
    class DHT:
        ...
        def On_Message(self, Command, Sub_Topic=None):
            ...
            if Command == "?":
                ...
                # If we read the values from the sensor they are avalible under self.Humidity and self.Temperature
                # Error logging done by Read so just check if true
                # if self.Read() == True:
                # # If we read the values from the sensor they are avalible under self.Humidity and self.Temperature
                # # Error logging done by Read so just check if true
                # if self.Read() == True:
                #     # Publish state takes average and round into account
                #     self.Publish_State()
            ...
            # Return current state as json
                ...
                # # Get json value, if false sensor read failed
                # Command = self.Get_json(Reset=True)
                # # Read from the sensor and report ok or failed
                # # Error logging done by read() in get_json
                # if Command != False:

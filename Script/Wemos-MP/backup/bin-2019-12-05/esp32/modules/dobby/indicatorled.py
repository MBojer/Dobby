...
class Init:
    ...
    def __init__(self, Dobby):
        ...
        if Dobby.Pin_Monitor.Pins["IO2"]['Owner'] != "IndicatorLED":
            self.Dobby.Log(1, "System", "IndicatorLED - Disabeled pin: IO2 - In owned by: " + str(Dobby.Pin_Monitor.Pins["IO2"]['Owner']))
            return
        ...
        ## Create Machine pin
        self.Pin = machine.Pin(self.Dobby.Pin_Monitor.To_GPIO_Pin('IO2'), machine.Pin.OUT)
        # Turn led off
        self.LED_Off()
    ...
    def Blink(self, Name):
        ...
        # Well if its not off then i guess its on
            ...
            if self.Active_Left == 0:
                # Reset so we trigger next to stop
                self.Reset
                # return so we dont trigger the timer again
                return

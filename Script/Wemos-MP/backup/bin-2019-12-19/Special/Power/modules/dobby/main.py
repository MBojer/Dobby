...
class Run:
    ...
    def __init__(self, Config):
        ...
        # What type of board we are running on
        # i havent found a bettery way they to see if we can import esp32
        # if failes we have a esp8266
        try:
            import esp32
        except ImportError:
            self.ESP_Type = 8266
        ...
        if self.wlan0.config('essid') != self.Config['WiFi_SSID']:
            ...
            # BUG: if we set hostname here that will crash the script
            # Set wifi hostname
        ...
        if self.Pin_Monitor.Get_Owner('D4') == 'IndicatorLED':
            # Import IndicatorLED
    ...
    def MQTT_Commands(self, Payload):
        ...
        # Publishes hostname used on Wifi
            ...
            self.Sys_Modules['IndicatorLED'].Enable('FindMe')
        
        # ++++++++++++++++++++++++++++++++++++++++ Find Me ++++++++++++++++++++++++++++++++++++++++
        # Blinks the indicator led 20 times if configured
    ...
    def Log(self, Level, Topic, Payload):
        ...
        # blink IndicatorLED if configured
        if self.IndicatorLED != None:
            # Check if we got a blink for this log level aka anythign else then Debug and Info
            if Level > 1:
                self.IndicatorLED.Enable('Log-' + Level_String)

...
class Init:
    ...
    def Read_Sensor(self, Pin_Name):
        ...
        try:
            ...
            # Log event
            self.Dobby.Log(0, "DS18B20/" + Pin_Name, "Unable to read pin. Error count: " + str(self.Sensor_Pins[Pin_Name]['Error']))

            # check if we reached max error counter aka 10
            if self.Sensor_Pins[Pin_Name]['Error'] >= 10:
                # Log event
                self.Dobby.Log(0, "DS18B20/" + Pin_Name, "Disabling pin. Max error count reached")
        ...
        for Key in self.ids:
            ...
            # We if get an keyerror the sensor wasent configures so reports unknown sensor 
                ...
                if Key not in self.Unconfigured:
                    ...
                    self.Dobby.Log(2, "DS18B20", "Unconfigured sensor on Pin: " + Pin_Name + " id: " + Key)
    ...
    class DS18B20:
        ...
        def On_Message(self, Command, Sub_Topic=None):
            ...
            if Command == "?":
                ...
                self.Dobby.Log(2, 'DS18B20/' + self.Name, "Unknown command: " + str(Command))
        ...
        def Store_Temperature(self, Temperature):
            ...
            for Key in self.Indicator:
                ...
                if self.Action_Check(Key, self.Indicator) == True:
                    try:
                        # Need to indicate
                        print("INDICATE HERER use 'system'")
                        print(self.Indicator[Key]['Name'])
                        print(self.Indicator[Key]['Times'])
                        print(self.Indicator[Key]['On_For'])
                        print(self.Indicator[Key]['Delay'])
                        print(self.Indicator[Key]['Repeat'])
                        

                        for name in self.Dobby.Modules['indicator'].Peripherals:
                            print("HPAT", name)
                        print(self.Dobby.Modules['indicator'].Peripherals[self.Indicator[Key]['Name']])
                        ...
                        print("Errororrsdgsdgfgfgfdsgsfgsdfgdsfgfdsg")
                        print("Errororrsdgsdgfgfgfdsgsfgsdfgdsfgfdsg")
                        print("Errororrsdgsdgfgfgfdsgsfgsdfgdsfgfdsg")
                        print("Errororrsdgsdgfgfgfdsgsfgsdfgdsfgfdsg")

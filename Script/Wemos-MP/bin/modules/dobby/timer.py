...
class Init:
    ...
    def Time_To_ms(self, Time):
        ...
        # If not already int check if we kmow the time state
        # Remember to do Time[:-1] to remove s/m/h at the end
        try: 
            if Time.lower().endswith("s") == True:
                return int(int(Time[:-1]) * 1000)
                
            elif Time.lower().endswith("m") == True:
                return int(int(Time[:-1]) * 60000)
                
            elif Time.lower().endswith("h") == True:
                return int(int(Time[:-1]) * 3600000)
        except ValueError:
            # pass so we trigger error
            pass
            
        # Log event - Error
        self.Dobby.Log(2, "System", "Unknown time string: " + str(Time))

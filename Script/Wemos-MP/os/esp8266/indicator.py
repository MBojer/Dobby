...
class Init:
    ...
    def __init__(self, Base, Config):
        ...
        self.Peripherals = {}
        # Log Event
        self.Base.Log(1, "Indicator", "Initializing")
        ...
        # Log event
        self.Base.Log(0, "Indicator", "Initialization complete")
    ...
    def Add(self, Name, Config):
        ...
        self.Peripherals[Name] = self.Indicator(self.Base, self, Name, Config)

        # Check if the Indicator is ok
        if self.Peripherals[Name].OK is False:
            # Issue with Indicator detected disabling it
            self.Base.Log(2, "Indicator/" + Name, "Issue during setup, removing the Indicator")
            # remove the name from Peripherals
            del self.Peripherals[Name]
        # Indicator ok
        else:
            # Subscribe to topic
            self.Base.MQTT_Subscribe(self.Base.Peripherals_Topic("Indicator", End=Name))
    ...
    class Indicator:
        ...
        # -------------------------------------------------------------------------------------------------------
        def __init__(self, Base, Main, Name, Config):
            # Referance to Base
            self.Base = Base
            ...
            # Log Event
            self.Base.Log(0, "Indicator/" + self.Name, "Initializing")

            # Check if we got the needed config
            try:
                dConfig.Check(Config, ['Pin'])
            except self.Base.Error:
                # Log error
                self.Base.Log(3, "Indicator/" + self.Name, "Missing config - unable to initialize")
                return
            ...
            try:
                self.Base.Pin_Monitor.Reserve(self.Pin, "Indicator-" + self.Name)
            except self.Base.Pin_Monitor.Error:
                # Pin in use unable to configure Indicator
                self.Base.Log(3, "Indicator/" + Name, "Unable to take ownership of Pin: " + self.Pin + " - Unable to initialize")
            ...
            self.Pin = self.Base.Pin_Monitor.To_GPIO_Pin(Config['Pin'])

            # create the output pin object
            self.Pin = machine.Pin(self.Pin, machine.Pin.OUT)

            # Check if the Base.timer module is loaded
            self.Base.Timer_Init()
            # Add a timer
            # 1 = Referance Name
            # 2 = Timeout
            # 3 = Callback
            # Note auto is active by creating <state> key with referance to timer in Auto dict
            self.Timer = self.Base.Sys_Modules['timer'].Add(
                "Indicator-" + self.Name,
                "0.5s",
                self.Ping,
                Logging=False
        ...
        def Add(self, Name, Left, On_For, Delay, Repeat=False):
            ...
            if Name not in self.Active.keys():
                ...
                # Log event
                self.Base.Log(0, 'Indicator/' + self.Name, "Added Indicator " + str(Name) + " - Number of: " + str(Left) + " On for: " + str(On_For) + " Delay between: " + str(Delay) + " Repeat: " + str(Repeat))
        ...
        def Remove(self, Name):
            ...
            # Log event
            if Removed == True:
                self.Base.Log(0, 'Indicator/' + self.Name, "Removed: " + str(Name))
            else:
                self.Base.Log(0, 'Indicator/' + self.Name, str(Name) + " not added")
        ...
        def End(self):
            ...
            if self.Active != {}:
                ...
                self.Timer.Set_Timeout(self.Pause)
                self.Timer.Callback = self.Ping
                self.Timer.Start()
        ...
        def Ping(self):
            ...
            if self.Pin.value() == 1:
                ...
                self.Timer.Set_Timeout(self.Active[self.Active_Order[0]]['Delay'])
                self.Timer.Start()
            ...
            # Indicator is off
                ...
                self.Timer.Set_Timeout(self.Active[self.Active_Order[0]]['On_For'])
        ...
        def On_Message(self, Payload, Sub_Topic=None):
            ...
            if self.OK == False:
                ...
                self.Base.Log(2, 'Indicator/' + self.Name, "Disabeled")
            ...
            if Payload.lower().startswith('add ') == True:
                ...
                try:
                    ...
                    self.Base.Log(2, 'Indicator/' + self.Name, "Invalud Indicator info provided")
                    return
            ...
            # Unknown command
                ...
                self.Base.Log(2, 'Indicator', "Unknown Indicator command: " + str(Payload))